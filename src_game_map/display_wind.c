...
int all_set(maps_t *maps, anime_t *anim, sfView *view)
{
    ...
    sfView_setCenter(view, maps->center);
    sfView_setSize(view, maps->size_map);
    sfSprite_setTexture(anim->livre, anim->text_livre, sfTrue);
    sfSprite_setScale(anim->livre, (sfVector2f){0.3f, 0.3f});
    return 0;
}
...
int disp_split(sfRenderWindow *w, maps_t *map, anime_t *ani, sfView *view)
{
    ...
    sfRenderWindow_drawSprite (w, ani->livre, NULL);
...
int display_window(my_rpg *rpg, sfRenderWindow *window)
{
    ...
    while (sfRenderWindow_isOpen(window)) {
        while_for_display(window, rpg, view, animation);
